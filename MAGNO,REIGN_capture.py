...
# Function to capture and save an image sample
def capture_img_sample(img):
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                for (ex, ey, ew, eh) in eyes:
                    ...
                    cv2.putText(eye_roi, "Eye", (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                    cv2.imwrite(f'Dataset/Eyes/Image.{img_id}.{eye_id}.jpg', eye_roi)

                    # If EAR is below a threshold, consider the eyes as closed
                    if ear < 0.2:  # Threshold for closed eyes, adjust based on testing
    ...
    if len(coordinate) == 4:
        ...
        user_id = 1
        cv2.imwrite(f"Dataset/Reign/Image.{user_id}.{img_id}.jpg", roi_img)
        img_id += 1
